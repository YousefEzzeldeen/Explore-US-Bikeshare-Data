    ...
    while True:
        ...
        try:
            if input_read.lower() in ['chicago','new york city','washington'] and input_type == 1:
                ...
                if input_type == 1:
                    print("\n\n Error! Please Choose From: Chicago, New York City, or Washington ? \n\n ")
                if input_type == 2:
                    print("\n\n Error! Please Choose From: january, february, march, april, may, june or all ? \n\n ")
                if input_type == 3:
                    print("\n\n Error! Please Choose From : satersay, sunday, ... friday, or all ? \n\n ")
        except ValueError:
...
def trip_duration_stats(df):
    ...
    print ("Total tarvel Time :" , total_travel_time)
...
def user_stats(df,city):
    ...
    if city != 'washington':
        ...
        print('Washington has no data about gender nor birth year')
    print("\nThis took %s seconds." % (time.time() - start_time))
    print('-'*40)
...
def load_raw(df):
    while True:
            try:
                raw = str(input("\n\nWould you like to show raw data ? Y or N \n\n"))
                if raw.lower() == "y" :
                    raw_data = df.sample(5)
                    print (raw_data)
                elif raw.lower() == "n" :
                    break
